...
Queue.prototype.serializeJob = function(job){
  ...
  if (job.kue){
    ...
    if (!serialized.id) serialized.id = job.kue.id;
    if (!serialized.workerId) serialized.workerId = job.kue.data.workerId;
    if (!serialized.state) serialized.state = job.kue.data.state;
    if (!serialized.batchId) serialized.batchId = job.kue.data.batchId;
    if (!serialized.progress) serialized.progress = job.kue.data.progress;
    if (!serialized.type) serialized.type = job.kue.data.type;
    if (!serialized.step) serialized.step = job.kue.data.step;
    if (!serialized.data && job.kue.data && job.kue.data.data) serialized.data = job.kue.data.data;
  }
...
Queue.prototype.updateJob = function(id, updated){

  var _this = this;

  return new Promise(function(resolve, reject){

    var busyJob = _this.getBusyJob(id);

    var timestamp = Date.now();

    if (!busyJob) return reject(new Error('job with id: ' + id + ' is not busy or does not exist'));
    ...
    if (updated.log)  busyJob.data.log('job: ' + '  ' + id + '  ' + timestamp.toString(), updated.log);
