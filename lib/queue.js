...
function Queue(options) {
  ...
  if (!options.stopTimeout) options.stopTimeout = 5000;
...
Queue.prototype.initialize = function ($happn) {
...
Queue.prototype.listen = function (callback, $happn) {
...
Queue.prototype.__connect = function (callback, $happn) {
...
Queue.prototype.emit = function (key, data, $happn) {
...
Queue.prototype.on = function (key, handler) {

  return this.__events.on(key, handler);
};

Queue.prototype.off = function (key, handler) {
...
Queue.prototype.__updateMetric = function (key, subkey, value, $happn) {
...
Queue.prototype.__updateStateMetric = function (key, subkey, value, $happn) {
  ...
  if (!this.__metrics[found]) this.__metrics[found] = {};

  if (!this.__metrics[found][subkey]) this.__metrics[found][subkey] = 0;

  this.__metrics[found][subkey] += value;
...
Queue.prototype.attach = function (options, $happn) {
...
Queue.prototype.detach = function (options, $happn) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    async.eachSeries(cloneArray(_this.__workers[jobType], true), function (worker, workerCB) {
...
Queue.prototype.__updateWorkerBusy = function (id, value, $happn) {
...
Queue.prototype.__updateJobBatch = function (jobData, batch, callback, $happn) {
...
Queue.prototype.__saveNewJob = function (options, batch, resolve, reject, $happn) {
...
Queue.prototype.createJob = function (options, $happn) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (options.batchId) {
      ...
      return _this.getBatch(options.batchId).then(function (batch) {
        ...
        _this.__saveNewJob(options, batch, resolve, reject, $happn);
      });
    }

    _this.__saveNewJob(options, null, resolve, reject, $happn);
  });
};
...
Queue.prototype.removeJobs = function (options) {
...
Queue.prototype.updateJobs = function (criteria, updates) {
...
Queue.prototype.searchJobs = function (options) {
...
Queue.prototype.jobCountByState = function (options) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (options.state == null) return reject(new Error('missing argument job options.state'));
...
Queue.prototype.updateBusyJob = function (updated, $happn) {
...
Queue.prototype.unAssignJobs = function (jobArray, jobType, workerId, jobId, $happn) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (jobArray == null || jobArray.length == 0) return resolve([]);

    var jobIndex = jobArray.length;

    async.eachSeries(cloneArray(jobArray, true), function (job, jobCB) {
...
Queue.prototype.reAssignJobs = function (jobType, workerId, $happn) {
...
Queue.prototype.__cleanupJobDB = function (id, jobErr, $happn) {

  var _this = this;

  kue.Job.get(id, function (err, job) {

    if (err) return _this.emit('cleanup-error', {jobId: id, error: err.toString()}, $happn);
    ...
    if (jobErr) {
      ...
      if (removeErr) return _this.emit('job-cleanup-remove-error', {jobId: id, error: removeErr.toString()}, $happn);

      _this.emit('job-cleanup-remove-ok', {jobId: id}, $happn);
    });
  });
};

Queue.prototype.__removeBusyJob = function (id, $happn) {

  var _this = this;

  var found = null;

  cloneArray(_this.__busyJobs).every(function (job, jobIndex) {
