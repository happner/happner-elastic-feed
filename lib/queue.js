...
function Queue(){
  ...
  Object.defineProperty(this, '__assignedJobs', {value:{}});

  Object.defineProperty(this, '__busyJobs', {value:[]});

  Object.defineProperty(this, '__workers', {value:{}});
...
Queue.prototype.attach = function(options){
  ...
  return new Promise(function(resolve, reject){
    ...
    if (!_this.__workers[options.type]) _this.__workers[options.type] = [];

    var worker = {id:workerId, busy:[]};

    _this.__workers[options.type].push(worker);

    _this.reAssignJobs(options.type);

    _this.__updateMetric('attached', options.type, 1);
...
Queue.prototype.detach = function(options){
  ...
  return new Promise(function(resolve){
    ...
    _this.__workers[type].every(function(worker, workerIndex){
      ...
      if (worker.id == options.id){
        ...
        //remove the worker
        _this.__workers[type].splice(workerIndex, 1);
    ...
    if (found)  {
      _this.__updateMetric('attached', type, -1);
...
Queue.prototype.findWorker = function(id){
  ...
  if (_this.__workers[type] == null) return null;
...
Queue.prototype.__updateWorkerBusy = function(id, value){
  ...
  if (worker) {
    worker.busy += value;
    this.__updateMetric('busy', type, value);
  }
};
...
Queue.prototype.pop = function(options){
  ...
  return new Promise(function(resolve, reject){
    ...
    if (_this.__assignedJobs[type] == null) return resolve(false);

    _this.__assignedJobs[type].every(function(job, jobIndex){
      ...
      if (job.workerId == options.workerId) {
        ...
        _this.__assignedJobs[type].splice(jobIndex, 1);
      }
      return found == null;
    });

    if (found) {
      _this.__updateMetric('pending', type, -1);
      _this.__updateMetric('busy', type, 1);
...
Queue.prototype.createJob = function(options){
  ...
  return new Promise(function(resolve, reject){
    ...
    _this.getBatch(options.batchId).then(function(batch){
      ...
      try{
        ...
        var jobData = {
          ...
          data: options.data,
          type: options.type,
          queueId: options.type + '_' + uuid.v4(),
          state: _this.JOB_STATE.PENDING,
          progress:0,
          step:""
        };

        _this.__validateJobBatch(jobData, batch);

        var job = _this.__queue.create(options.type, jobData).save(function(e){

          if (e) return reject(e);

          _this.__updateJobBatch(job, batch).then(function(){

            _this.__updateMetric('pending', options.type, 1);
...
Queue.prototype.initialize = function(options){
  ...
  return new Promise(function(resolve, reject){
    ...
    _this.__connect(options, function(e){
      if (e) return reject(e);
      resolve();
    });
  });
  ...
  Object.keys(_this.JOB_TYPE).forEach(function(jobType){
    ...
    _this.__queue
      .process(_this.JOB_TYPE[jobType], _this.options.concurrency[jobType], function(job, ctx, done){
        ...
        _this.__assignJob(job.data.type, job, ctx, jobDone);
      });
  });
...
Queue.prototype.stop = function(options){
  ...
  return new Promise(function(resolve){
    ...
    if (!_this.__queue) return resolve();

    _this.__queue.shutdown(options.timeout, function(e) {
...
Queue.prototype.__connect = function(options, callback){
  ...
  if (options.deferListen) return callback();
