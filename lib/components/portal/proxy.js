...
function Proxy(options) {
  ...
  options.wildProxyConfig = {

    listeners:[
      {
        name: 'elastic-proxy',
        port: options.proxy.elasticListenPort,
        protocol:options.proxy.elasticProtocol,
        target:  options.proxy.elasticURL
      },
      {
        name: 'kibana-proxy',
        port: options.proxy.kibanaListenPort,
        protocol: options.proxy.kibanaProtocol,
        target:options.proxy.kibanaURL
      }
    ],
    rules:[
      {
        name: 'elastic-proxy',
        path: '*',
        handler: _this.__handleElasticRequest.bind(_this),
        terminate: true
      },
      {
        name:'kibana-proxy',
        steps:[
          {
            ...
            path: '/auth?*',
            handler: _this.__handleKibanaAuthenticate.bind(_this),
            terminate: true
          },
          {
            name: 'kibana-authorize',
            path: '*',
            handler: _this.__handleKibanaAuthorize.bind(_this)
          },
          {
            name: 'kibana-available-dashboards',
            path: '/dashboards?*',
            handler: _this.__handleKibanaAvailableDashboards.bind(_this),
            terminate: true
          },
          {
            name: 'kibana-proxy',
            path: '/app/kibana*',
            handler: _this.__handleKibanaRequest.bind(_this),
            terminate: true
          },
          {
            name: 'kibana-bad',
            path: '*',
            handler: _this.__handleKibanaBadRequest.bind(_this)
          }
        ]
      }
    ]
...
function initialize($happn) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this.__proxy = new WildProxy(_this.__options.wildProxyConfig);
...
function __handleElasticRequest(proxyReq, req, res, options) {
  ...
  return new Promise(function(resolve, reject){
    ...
    try {
      ...
      _this.instance.emit('handle-request-happening', {
        from: req.url,
        to: _this.instance.__options.proxy.target
      }, _this.$happn);
...
function __handleKibanaAuthenticate(proxyReq, req, res, options) {
  ...
  return new Promise(function(resolve) {
    ...
    console.log('kibana-authenticate:::');

    _this.instance.emit('kibana-authenticate', req.url, _this.$happn);

    if (!req.uri.query || !req.uri.query.happn_token) return _this.__respond(res, null, null, new Error('missing happn_token querystring argument'));

    if (!req.uri.query || !req.uri.query.redirect) return _this.__respond(res, null, null, new Error('missing redirect querystring argument'));

    var redirect = decodeURIComponent(req.uri.query.redirect);

    var header = {
      'Set-Cookie': 'happn_token=' + req.uri.query.happn_token
    };

    return _this.__respond(res, header, null, null, redirect);
  });
}

function __handleKibanaAuthorize(proxyReq, req, res, options) {

  var _this = this;

  return new Promise(function(resolve){

    console.log('request:::kibana-authorize');
...
function __handleKibanaAvailableDashboards(proxyReq, req, res, options) {

  var _this = this;

  return new Promise(function(resolve){

    console.log('request:::kibana-available-dashboards');
...
function __handleKibanaBadRequest(proxyReq, req, res, options) {

  var _this = this;

  return new Promise(function(resolve){

    console.log('request:::kibana-bad-request');
...
function __handleKibanaRequest(proxyReq, req, res, options) {

  var _this = this;

  return new Promise(function(resolve){

    console.log('request:::kibana-request');
