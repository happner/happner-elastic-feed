...
function __handleKibanaAuthorize(req, res) {
  ...
  return new Promise(function (resolve) {
    ...
    var token = __getHappnToken(req, res, true);

    if (!token){

      _this.instance.emit('kibana-authorize-failed-no-token', req.url, _this.$happn);
...
function __handleElasticAuthorize(req, res) {
  ...
  return new Promise(function (resolve) {
    ...
    var token = __getHappnToken(req, res);
    ...
    utilities.processPost(req, res)
      ...
      .then(function () {
        ...
        if (!token){
          ...
          if (_this.instance.__checkPassthrough(req)) return resolve();

          _this.instance.emit('elastic-authorize-failed-no-token', req.url, _this.$happn);
    ...
    async.eachSeries(Object.keys(user.groups), function(groupName, groupCB){
      ...
      _this.__happnSecurityService.users.getGroup(groupName, function (e, group) {
        ...
        for (var permissionPath in group.permissions){
          ...
          if (permission.actions.indexOf('on') > -1 || permission.actions.indexOf('get') > -1 || permission.actions.indexOf('*') > -1){
            _this.__permissions.add(permissionPath, {key:session.id, data:session})
          }
        }
