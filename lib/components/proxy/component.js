...
function __handleKibanaAuthorize(req, res) {
  ...
  return new Promise(function (resolve) {
    ...
    var token = _this.instance.__getHappnToken(req, res, true);//throw 403 is true, so response is sent

    if (!token) _this.instance.emit('kibana-authorize-failed', {url:req.url, headers:req.headers}, _this.$happn);

    return resolve(!token);//stop executing if the token doesn't exist, (!null = true)
  });
}
