...
function __handleKibanaAuthenticate(req, res) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var redirect = authURL.query.redirect ? decodeURIComponent(authURL.query.redirect) : '/app/kibana';

    //TODO: move doFail and doRedirect to methods on the class

    var doFail = function (event, error, code) {

      if (!code) code = 403;

      _this.instance.__logEvent('kibana-authenticate-failed', error, true, _this.$happn);
    ...
    if (!token && !username) {
      ...
      return doFail('kibana-authenticate-failed', new Error('missing happn_token or username querystring arguments'));
    }
    ...
    if (username) {
      ...
      if (!password) return doFail('kibana-authenticate-failed', new Error('missing password querystring argument'));
      ...
      return meshClient.login({
          ...
          _this.instance.__sessionFromToken(meshClient.token, _this.$happn)
            ...
            .then(function (session) {
              ...
              _this.instance.__setPermissions(session, _this.$happn, function (e) {
                ...
                if (e)
                  return doFail('kibana-authenticate-failed', e);
        ...
        .catch(function (e) {
          ...
          return doFail('kibana-authenticate-failed', new Error('__kibanaAuthenticate failed, credentials auth error: ' + e.toString()), 500);
        });
    }
    ...
    _this.instance.__authorizeEngine.enforce('browser-kibana-authorize', {req: req, res: res},
      ...
      function (ruleName, facts, consequence, callback) {
        ...
        if (consequence && consequence.action == "deny") {
          ...
          _this.instance.__logEvent('kibana-authorize-failed', {
            url: req.method + ":" + req.url,
            reason: ruleName + ":" + consequence.message
          }, true, _this.$happn);
        ...
        if (consequence && consequence.action == "proxy") {
          ...
          _this.instance.__logEvent('kibana-authorize-succeeded', {
            url: req.method + ":" + req.url,
            reason: ruleName + ":" + consequence.message
          }, true, _this.$happn);
    ...
    _this.instance.__authorizeEngine.enforce('kibana-elastic-authorize', {req: req, res: res},
      ...
      function (ruleName, facts, consequence, callback) {
        ...
        if (consequence && consequence.action == "deny") {
          ...
          _this.instance.__logEvent('elastic-authorize-failed', {
            url: req.method + ":" + req.url,
            reason: ruleName + ":" + consequence.message
          }, true, _this.$happn);
        ...
        if (consequence && consequence.action == "allow") {
          ...
          _this.instance.__logEvent('elastic-authorize-succeeded', {
            url: req.method + ":" + req.url,
...
function __logEvent(event, data, emit, $happn) {
  ...
  if (this.__options.proxy.log_output) {
    ...
    try {
      ...
      var wasError = event.indexOf('failed') > -1;

      if ((this.__options.proxy.log_output_errors_only && wasError) || !this.__options.proxy.log_output_errors_only) {

        var message = utilities.rpad(Date.now(), 20) + utilities.rpad(event, 30) + utilities.rpad(data != null ? JSON.stringify(data) : '', 100);

        if (wasError) message = colors.red(message);
        else message = colors.green(message);

        console.log(message);
      }
    } catch (e) {

      console.log('failed logging event: ', e.toString());
    }
  }
}
