...
function __handleKibanaAuthenticate(req, res) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (username) {
      ...
      if (!password){
        _this.instance.__logEvent('kibana-authenticate-failed', 'missing password querystring argument');
        return reject(new Error('missing password querystring argument'));
      }
          ...
          _this.instance.__sessionFromToken(meshClient.token, _this.$happn)
            ...
            .then(function (session) {
              ...
              _this.instance.__setPermissions(session, _this.$happn, function(e){
                ...
                if (e) {
                  _this.instance.__logEvent('kibana-authenticate-failed', '__setPermissions:' + e.toString());
                  return reject(e);
                }
