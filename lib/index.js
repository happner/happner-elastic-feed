...
function __parseBaseConfig(config) {
  ...
  var SECURITY_COOKIE_NAME = process.env.SECURITY_COOKIE_NAME;

  if (SECURITY_COOKIE_NAME == null)
    SECURITY_COOKIE_NAME = 'happn_token';

  var SECURITY_COOKIE_DOMAIN = process.env.SECURITY_COOKIE_DOMAIN;
  ...
  var __happnConfig = {
    ...
    services: {
      security: {
        config: {
          cookieName: SECURITY_COOKIE_NAME,
          cookieDomain: SECURITY_COOKIE_DOMAIN,
          sessionTokenSecret:SESSION_TOKEN_SECRET,
          adminUser: {
            password: CREDS_DATA_PASSWORD
          }
        }
      },
      data: {
  ...
  console.log('happn config:::', JSON.stringify(__happnConfig, null, 2));
