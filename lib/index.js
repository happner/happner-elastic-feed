var Promise = require('bluebird')
  ...
  , Analyzer = require('./analyze/service')
  ;
...
function __requireComponents (options){
  ...
  if (options.methodDurationMetrics){
    ...
    this.methodAnalyzer = new Analyzer();
    ...
    this.Proxy = this.methodAnalyzer.require('./components/portal/proxy', true);
    ...
    this.Proxy = require('./components/portal/proxy');
...
function __parseBaseConfig (config) {
  ...
  var hapnnerConfig = {
    ...
    modules: {
      ...
      "feed": {
        instance: this.instantiateServiceInstance(_this.Feed, config.feed)
      }
    },
    components: {
      "service": {
  ...
  baseConfig.components.proxy = {
    web: {
      routes: {
        ...
        "/dashboard/list/authorized": ["dashboardListAuthorized"]
      }
    },
    startMethod: "initialize",
    stopMethod: "stop"
